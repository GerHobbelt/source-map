...
class IndexedSourceMapConsumer extends SourceMapConsumer {
   ...
   */
  get sources() {
    console.warn("get sources:", this._sections);
    const sources = [];
  ...
  originalPositionFor(aArgs) {
    ...
    console.warn("originalPositionFor CALL:", needle);
    ...
    const sectionIndex = binarySearch.search(needle, this._sections, function(
      ...
      console.warn(
        "\nbinarySearch SECTIONS:",
        {
          needle: aNeedle,
          'section.generatedOffset': section.generatedOffset,
          compareCheck: cmp,
          compareValue2: aNeedle.generatedColumn - (section.generatedOffset.generatedColumn - 1)
      });
      if (cmp) {
        return cmp;
      }
    ...
    console.warn("binarySearch SECTIONS --> found @ index:", { sectionIndex, section });
